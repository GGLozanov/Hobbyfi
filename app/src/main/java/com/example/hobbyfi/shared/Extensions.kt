...
fun FormBody.Builder.addFetchRegisterTokenRequest(
    email: String?, password: String?, username: String, description: String?,
    tags: List<Tag>
): FormBody.Builder {
    addEncoded(Constants.USERNAME, username)

    email?.let {
        addEncoded(Constants.EMAIL, it)
    }

    description?.let {
        addEncoded(Constants.DESCRIPTION, it)
    }

    password?.let {
        addEncoded(Constants.PASSWORD, it)
    }

    if(tags.isNotEmpty()) {
        addEncoded(Constants.TAGS, Constants.jsonConverter.toJson(tags))
    }
